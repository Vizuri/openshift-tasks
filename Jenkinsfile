      ...
      steps {
            container("skopeo") {
                sh  '''
                  ...
                  skopeo --debug copy --src-creds="$(oc whoami)":"$(oc whoami -t)" --src-tls-verify=false --dest-tls-verify=false --dest-creds=student1:workshop1! docker://docker-registry.default.svc:5000/dev-student1/task:latest docker://quay.apps.ocpws.kee.vizuri.com/student1/tasks:latest
                '''
            }
        }
    }
